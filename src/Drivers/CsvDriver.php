...
class CsvDriver implements FileDriver
{
	...
	{
		return $mapping->isFileSource(static::TYPE);
	}
	...
	{
		$config = $this->getConfig($mapping);

		return $config['alias'] ?? 'csvsrc';
	}
	 ...
	 *
	 */
	protected function getConfig(Mapping $mapping)
	{
		return array_merge([
			'header'    => TRUE,
			'delimiter' => ',',
			...
			'columns'   => [],
			'alias'     => 'csvsrc'
		], $mapping->getFileConfig(static::TYPE) ?: []);
	}
}
